      ...
      <div className="bg-muted/30 py-12 border-b border-border">
        <div className="container mx-auto px-6 max-w-4xl">
            ...
            {[1, 2, 3, 4].map((s) => (
              <div key={s} className={`relative z-10 flex flex-col items-center gap-2 ${step === 5 && s === 4 ? 'opacity-0' : ''}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                  step > s ? 'bg-secondary text-white border-secondary' : 
          ...
          <div className="bg-background border border-border p-6 md:p-10 shadow-sm min-h-[500px]">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {serviceTypes.map(srv => (
                    ...
                    >
                      ...
                      <div className="text-xl font-serif text-primary mb-1">
                        {srv.price === 0 ? 'Free' : `₦${srv.price.toLocaleString()}`}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3 pt-6 border-t border-border">
                  ...
                  >
                    ...
                    {practiceAreas.map(pa => (
                      <option key={pa.slug} value={pa.title}>{pa.title}</option>
                    ))}
                  </select>
                </div>
            ...
            {step === 2 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                <div className="flex items-center gap-4 mb-6">
                  <Button variant="ghost" size="icon" onClick={prevStep} className="h-8 w-8 rounded-full">
            ...
            {/* STEP 4: PAYMENT (PAYSTACK STYLED) */}
            {step === 4 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-right-4 max-w-2xl mx-auto">
                ...
                <div className="bg-[#f9f9f9] border border-[#e4e4e4] rounded-lg overflow-hidden shadow-sm">
                  ...
                  <div className="p-6 bg-white">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase">Valid Till</label>
                        <Input placeholder="MM/YY" className="mt-1 bg-gray-50 border-gray-200" />
                      </div>
                      <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase">CVV</label>
                        <Input placeholder="123" className="mt-1 bg-gray-50 border-gray-200" type="password" maxLength={3} />
                      </div>
                    </div>
                    <Button 
                ...
                <div className="text-center text-xs text-muted-foreground mt-4">
                  Note: This is a UI mockup of Paystack checkout — connect a Paystack account to process live payments.
                </div>
              </div>
            )}
            ...
            {step === 5 && (
              <div className="text-center py-12 animate-in zoom-in-95 space-y-6 max-w-xl mx-auto">
                ...
                <p className="text-muted-foreground text-lg">
                  Your {serviceObj?.name} has been scheduled successfully.
                </p>
                ...
                <p className="text-sm text-muted-foreground mb-8">
                  A confirmation email with calendar invites and meeting links will be sent to <strong>{details?.email}</strong>.
                </p>

                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button variant="outline" className="border-border rounded-none h-12" onClick={() => alert("ICS Download triggered in production")}>
                  ...
                  <a href={`https://wa.me/2347054588490?text=Hi, I just booked a ${serviceObj?.name} for ${selectedDate ? format(selectedDate, 'MMM do') : ''} at ${selectedTime}. Ref: ${refCode}`} target="_blank" rel="noreferrer">
                    <Button className="w-full bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-none h-12">
